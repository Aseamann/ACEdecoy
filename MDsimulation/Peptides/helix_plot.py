...
def read_scount(file_in, multi=False):
    """
    ...
    if multi:
        ...
        files = [file_in]
    for file_i in files:
        number = input("Number of residues for structure " + file_i.split("/")[-1].split(".")[0] + ": ")
        with open(file_i, "r") as f1:
            structure_data = {}  # {structure: [0, 1]}
            for line in f1:
                if re.match("^@ s\d", line[0:4]) or re.match('^@ subtitle "Backbone after lsq fit to Backbone"', line):
                ...
                if len(structure_data.keys()) != 0 and line[0] == " ":
                    ...
                    for i in range(len(structure_data.keys())):
                        ...
                        if list(structure_data.keys())[i] != "Backbone after lsq fit to Backbone":
                            structure_data[list(structure_data.keys())[i]].append(int(data[i + 1]) / int(number))
                        else:
                            structure_data[list(structure_data.keys())[i]].append(float(data[i + 1]))
            if multi:
...
def plot_structure(data, structure, multi=False):
    ...
    plt.ylim(0, 1)
    if structure == "Backbone after lsq fit to Backbone":
        plt.ylabel("RMSD")
        plt.ylim(0,1)
    else:
...
def main():
    ...
    if args.structure == "RMSD":
        structure = "Backbone after lsq fit to Backbone"
    if os.path.isfile(args.scount_file):
        structure_data = read_scount(args.scount_file)
        plot_structure(structure_data, structure)
    elif os.path.isdir(args.scount_file):
        structure_data = read_scount(args.scount_file, True)
        plot_structure(structure_data, structure, True)
